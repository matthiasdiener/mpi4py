...
from mpi4py.futures.util import gather
...
from mpi4py.futures.util import resolve
...
class ResolveTest(unittest.TestCase):
    ...
    def test_result_hook(self):
        base = futures.Future()
        future = resolve(base, int)
        base.set_result('42')
        self.assertEqual(future.result(), 42)

    def test_result_hook_failure(self):
        base = futures.Future()
        future = resolve(base, resulthook=lambda x: 1/0)
        base.set_result(42)
        self.assertIs(type(future.exception()), ZeroDivisionError)

    def test_except_hook(self):
        base = futures.Future()
        future = resolve(base, excepthook=lambda exc: exc.args[0])
        base.set_exception(RuntimeError(42))
        self.assertEqual(future.result(), 42)

    def test_except_hook_except(self):
        base = futures.Future()
        future = resolve(base, excepthook=lambda exc: RuntimeError(exc.args[0]))
    ...
    def test_except_hook_failure(self):
        base = futures.Future()
        future = resolve(base, excepthook=lambda exc: 1/0)
        base.set_exception(ValueError(42))
        self.assertIs(type(future.exception()), ZeroDivisionError)
