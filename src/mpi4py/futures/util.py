...
def wrap(future, resulthook=None, excepthook=None):
    """Wrap the completion of a future with result and exception handlers.
    ...
    Args:
        ...
        excepthook: Function to be called once the input future completes with
            ...
            exception value is the input argument for *excepthook*.
            If *excepthook* returns an `Exception` instance, it is set as the
            exception of the output future.  Otherwise, the result of
            *excepthook* is set as the result of the output future.
            If *excepthook* is ``None``, the output future is set as failed
            with the exception from the input future.

    Returns:
        Output future instance to be completed once the input future is
        completed and either *resulthook* or *excepthook* completes.

    """
    new_future = future.__class__()
    new_done_cb = _functools.partial(_wrap_cancel, [future])
    new_future.add_done_callback(new_done_cb)
    context = [(new_future, resulthook, excepthook)]
    done_cb = _functools.partial(_wrap_complete, context)
    future.add_done_callback(done_cb)
    return new_future


def _wrap_complete(context, old_future):
...
def _wrap_cancel(context, new_future):
